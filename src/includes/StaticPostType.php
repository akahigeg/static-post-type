...
class StaticPostType
{
    ...
    public static function enableManageCustomColumns()
    {
        ...
        foreach ($post_types as $post_type => $options) {
            ...
            add_action($hook_name, 'StaticPostType::manageCustomColumns', 10, 2);
        }
    }
        ...
        foreach ($post_types as $post_type_name => $options) {
                ...
                foreach ($options['meta_boxes'] as $index => $meta_box) {
                    foreach ($meta_box as $name => $meta_box_options) {
                        ...
                        if (isset($meta_box_options['rendering_function']) && function_exists($meta_box_options['rendering_function'])) {
                            ...
                            $rendering_meta_box_function = 'StaticPostType::renderMetaBox';
                        }

                        $include_fields = $meta_box_options['fields'];
                        add_meta_box(
                          $name . '_meta_box_options',
            ...
            if (array_key_exists('custom_fields', $options)) {
                ...
                if (count($include_fields) > 0) {
                    add_meta_box(
                       $post_type_name. '_meta_box',
                       'Custom Fields',
                       'StaticPostType::renderMetaBox',
                       $post_type_name,
                       'normal',
                       'default',
                       $include_fields
     ...
     *                 ["callback"]=> string(23) "StaticPostType::renderMetaBox"
        ...
        if (array_key_exists($post_type_name, $post_types)) {
            ...
            foreach ($custom_fields as $custom_field) {
                foreach ($custom_field as $name => $options) {
                    ...
                    echo '<div class="static-post-type-inner-meta-box">';
                    ...
                    if (isset($options['rendering_function']) && function_exists($options['rendering_function'])) {
                        ...
                        $class_name = 'StaticPostTypeFormRenderer' . StaticPostTypeUtil::pascalize($options['input']);
            ...
            if (isset($options['custom_fields'])) {
                foreach ($options['custom_fields'] as $i => $custom_field) {
                    foreach ($custom_field as $name => $field_options) {
                        ...
                        $class_name = 'StaticPostTypeFormRenderer' . StaticPostTypeUtil::pascalize($field_options['input']);
        ...
        foreach ($post_types as $post_type_name => $options) {
            if (isset($options['sortable_columns'])) {
                ...
                if ($count > 0) {
                    ...
                    add_filter('manage_edit-' . $post_type_name . '_sortable_columns', 'StaticPostType::sortableColumns');
                }
            }
        }
    }
    ...
    {
        wp_enqueue_style('static-post-type-style', plugins_url('../css/static-post-type.css', __FILE__));
        if (file_exists(get_stylesheet_directory() . '/static-post-type.css')) {
            wp_enqueue_style('static-post-type-theme-style', get_stylesheet_directory_uri() . '/static-post-type.css');
        }
    }
    ...
    public static function script()
    {
        ...
        wp_enqueue_script('static-post-type_update_reference', plugins_url('../js/update_reference.js', __FILE__));
    }
    ...
    {
        add_action('init', 'StaticPostType::init');
        add_action('add_meta_boxes', 'StaticPostType::addMetaBoxes');
        add_action('save_post', 'StaticPostType::saveMeta');

        // TODO: testing for taxonomy custom field and uploading image

        add_action('created_term', 'StaticPostType::saveTermMeta');
        add_action('edited_term', 'StaticPostType::saveTermMeta');

        add_action('manage_posts_columns', 'StaticPostType::manageColumns');
        add_action('manage_pages_columns', 'StaticPostType::manageColumns');
        self::enableManageCustomColumns();

        add_action('admin_enqueue_scripts', 'StaticPostType::enqueueStyle');
        add_action('admin_enqueue_scripts', 'StaticPostType::enqueueScript');

        add_action('wp_ajax_static-post-type', 'StaticPostType::ajax');
        add_action('admin_enqueue_scripts', 'StaticPostType::script');
    }
                    ...
                    if (is_array($taxonomy_config)) {
                        foreach ($taxonomy_config as $name => $options) {
                            if (!empty($options) && array_key_exists('custom_fields', $options)) {
                                add_action($name . '_add_form_fields', 'StaticPostType::addTaxonomyMetaBoxForAdd');
                                add_action($name . '_edit_form', 'StaticPostType::addTaxonomyMetaBoxForEdit');
                            }
                        }
                    }
                }
...
function spt_example_rendering_func($name, $saved_value, $options)
{
...
function spt_example_rendering_meta_box_func($post, $args)
{
    $post_type_name = get_post_type($post);
    $custom_field_values = get_post_custom();
...
function spt_example_save_func($post_id, $name, $options)
{
    if (isset($_POST[$name])) {
        $value = $_POST[$name] . '+';
    } else {
